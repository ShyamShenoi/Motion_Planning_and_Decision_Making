...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    double cur_x = wp.x;
    double cur_y = wp.y;
    double cur_yaw = wp.theta;  // radians

    for (size_t c = 0; c < n_circles && !collision; ++c) {
      // Place ego circles along the vehicle body at offset c in the heading direction
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // Collision when circles overlap (sum of radii threshold)
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
                                      State main_goal) {
  // Use the last spiral point to measure distance to the center lane goal.
  // Spirals ending closer to the lane center-line are preferred.
  auto n = spiral.size();
