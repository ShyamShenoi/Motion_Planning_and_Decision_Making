...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  // Lookahead = speed * time, clamped to [min, max]
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  // Improvement 1: Decrement lane change cooldown each tick
  if (_lane_change_cooldown > 0) {
    --_lane_change_cooldown;
  }
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    if (is_goal_in_junction) {
      ...
      // Place the goal slightly behind the stop line so we stop before it
      ...
      // Speed at the stopping point is zero
      goal.velocity.x = 0.0;
      goal.velocity.y = 0.0;
      goal.velocity.z = 0.0;

    } else {
      // Nominal driving: set goal speed to speed limit in heading direction
    ...
    // Maintain the same goal (stop line position) throughout deceleration
    goal = _goal;

    auto distance_to_stop_sign =
        utils::magnitude(goal.location - ego_state.location);

    // Transition to STOPPED when close enough to the stop line
    ...
    // Maintain position at the stop line
    goal = _goal;
  ...
  } else if (_active_maneuver == LANE_CHANGE_LEFT ||
             _active_maneuver == LANE_CHANGE_RIGHT) {
    // Once a lane change is initiated, set goal speed for lane-change travel
    goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw);
    goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw);
    goal.velocity.z = 0.0;

    // Return to FOLLOW_LANE after one planning tick
    // (the motion planner's offset goal selection handles the actual lateral shift)
    _active_maneuver = FOLLOW_LANE;
    _lane_change_cooldown = P_LANE_CHANGE_COOLDOWN_TICKS;
  }
