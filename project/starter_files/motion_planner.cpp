...
State MotionPlanner::get_goal_state_in_ego_frame(const State& ego_state,
  ...
  // Translate so the ego state is at the origin
  ...
  // Rotate by -ego_yaw so ego heading becomes theta = 0
  ...
  goal_state_ego_frame.location.x = cos_theta * goal_x - sin_theta * goal_y;
  goal_state_ego_frame.location.y = sin_theta * goal_x + cos_theta * goal_y;
...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  // Perpendicular direction: add pi/2 to the goal heading
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    // Offset the goal laterally (perpendicular to heading direction)
...
std::vector<int> MotionPlanner::get_best_spiral_idx(
  ...
  for (size_t i = 0; i < spirals.size(); ++i) {
    ...
    bool is_collision = std::isinf(cost);

    // Improvement 3: Path validity logging
    std::cout << "[MotionPlanner] Path " << (i + 1) << "/" << spirals.size()
              << ": cost=" << (is_collision ? std::numeric_limits<double>::infinity() : cost)
              << " " << (is_collision ? "COLLISION" : "VALID") << std::endl;

    if (is_collision) {
      collisions.push_back(static_cast<int>(i));

      // Improvement 4: Track which colliding path is furthest from all obstacles
      double min_dist = DBL_MAX;
      for (const auto& wp : spirals[i]) {
        for (const auto& obs : obstacles) {
          double dx = wp.x - obs.location.x;
          double dy = wp.y - obs.location.y;
          double d = std::sqrt(dx * dx + dy * dy);
          if (d < min_dist) min_dist = d;
        }
      }
      if (obstacles.empty()) min_dist = 0.0;
      if (min_dist > max_min_dist) {
        max_min_dist = min_dist;
        emergency_idx = static_cast<int>(i);
      }
    } else {
      if (cost < best_cost) {
        best_cost = cost;
        best_spiral_idx = static_cast<int>(i);
      }
    }
  }

  if (best_spiral_idx != -1) {
    // Last element is the chosen (collision-free) best path
    collisions.push_back(best_spiral_idx);
    return collisions;
  }

  // Improvement 4: Emergency fallback when all paths collide
  if (emergency_idx != -1) {
    std::cout << "[MotionPlanner] WARNING: All paths in collision. "
                 "Using emergency fallback (path "
              << emergency_idx + 1 << "/" << spirals.size() << ")." << std::endl;
    collisions.push_back(emergency_idx);
    return collisions;
  }
